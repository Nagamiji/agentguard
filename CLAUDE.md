...
## Current state
See `STATE.md` for the live snapshot. On `main`: multi-tenancy, agent registry + fingerprint, evaluation engine + gate, real Vertex execution, the failure scenario library + risk report, the policy engine, and (this cycle) the deployment-gate CLI + GitHub Action.

## Where to build next
Validated 2026-07-17: `agentguard scan` over real HTTP blocked a deploy with exit code 20 and wrote valid SARIF — the product loop closed end to end (register → scan → policy-aware gate → CI blocks the merge).
Next, to make the wedge adoptable and harder to bypass: **publish the CLI** to a package index; **async scans + push-mode webhooks**; **asymmetric verdict signing** (Ed25519) for third-party attestation; the policy **`locked` ceiling**; an **`agent.project_id` link** (unlocks project scope); and **growing the attack library** (coverage is still the real constraint — a dashboard is chrome until then). One task per loop iteration.
