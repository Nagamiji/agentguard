...
## Current state
See `STATE.md` for the live snapshot. On `main`: multi-tenancy, agent registry + fingerprint, evaluation engine + gate, real Vertex execution, the failure scenario library + risk report, and (this cycle) the policy engine.

## Where to build next
Validated 2026-07-17: a live `gemini-2.5-flash` attempted a $9,000 refund and an **org policy** (not a scenario check) blocked it — the engine consuming a compiled policy against a real model.
Next: **Phase 5 — the deployment gate.** A GitHub Action + CLI that calls `GET /agents/{id}/gate` and `/risk` in CI, blocks the merge on `blocked`, and emits SARIF so findings show up in the PR. Everything it needs exists server-side; Phase 5 is the client + CI surface — what makes this a control a developer actually feels. Then the policy `locked` ceiling, an `agent.project_id` link (unlocks project scope), and growing the library. One task per loop iteration.
